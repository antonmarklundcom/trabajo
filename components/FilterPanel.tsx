  ...
  const FilterContent = () => (
    <div className="space-y-6">
      {hasActiveFilters && (
        <button
          onClick={clearAll}
          className="text-xs text-[#2557D6] hover:underline font-medium"
        >
          Limpiar filtros
        </button>
      )}
      ...
      <FilterSection label="Categoría">
        <select
          ...
          className="w-full px-3 py-2 rounded-[10px] border border-[#E5E7EB] text-sm text-[#16181D] bg-white focus:outline-none focus:border-[#2557D6] focus:ring-1 focus:ring-[#2557D6]"
        >
    ...
    <>
      ...
      <div className="lg:hidden mb-4">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-[10px] border border-[#E5E7EB] bg-white text-sm font-medium text-[#16181D] hover:border-[#2557D6] transition-colors"
        >
          ...
          Filtros
          {hasActiveFilters && (
            <span className="ml-1 w-2 h-2 rounded-full bg-[#2557D6]" aria-hidden="true" />
          )}
        </button>
      </div>
      ...
      <aside className="hidden lg:block w-64 flex-shrink-0">
        <div className="bg-white rounded-[10px] border border-[#E5E7EB] p-5 sticky top-24">
          <h2 className="text-sm font-semibold text-[#16181D] mb-4">Filtros</h2>
          <FilterContent />
        </div>
      </aside>
        ...
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl shadow-xl p-6 lg:hidden max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-semibold text-[#16181D]">Filtros</h2>
              <button
                onClick={() => setDrawerOpen(false)}
                className="w-8 h-8 flex items-center justify-center rounded-full bg-[#F7F8FA] text-[#5B6472] hover:bg-[#E5E7EB] transition-colors"
            ...
            <button
              onClick={() => setDrawerOpen(false)}
              className="mt-6 w-full py-3 rounded-[10px] bg-[#2557D6] text-white font-semibold hover:bg-[#1E47B8] transition-colors"
            >
              Ver resultados
            </button>
          </div>
...
function FilterSection({
  ...
  return (
    <div>
      <label className="block text-xs font-semibold uppercase tracking-wider text-[#5B6472] mb-2">
        {label}
      </label>
      {children}
    </div>
