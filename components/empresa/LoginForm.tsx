...
export default function LoginForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      // Reuses the existing staff/employer login endpoint on purpose — it
      // already returns redirectTo from homePathForRole() and shares the one
      // rate-limit budget for the `users` table. A second endpoint here would
      // split that budget in two for the same accounts.
      ...
      // The server decides the destination — it is the only side that knows
      // the role, and it must never send an employer into /admin.
