  ...
  return (
    <div className="space-y-4">
      {experiences.length === 0 && !adding && (
        <p className="text-sm text-ink-secondary">Todavía no cargaste experiencia laboral.</p>
      )}

      <ul className="space-y-3">
        {experiences.map((exp) => (
          <li key={exp.id} className="border border-border rounded-[10px] p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-medium text-ink">{exp.title}</p>
                <p className="text-sm text-ink-secondary">{exp.companyName}</p>
                <p className="text-xs text-ink-3 mt-1">
                  {formatMonth(exp.startMonth)} — {exp.isCurrent ? 'Actual' : exp.endMonth ? formatMonth(exp.endMonth) : ''}
                </p>
                {exp.description && (
                  <p className="text-sm text-ink-secondary mt-2 whitespace-pre-line">{exp.description}</p>
                )}
              </div>
      ...
      {adding ? (
        <form onSubmit={handleSubmit} className="border border-border rounded-[10px] p-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-ink mb-1">Puesto</label>
              <input
                type="text"
                required
                value={values.title}
            ...
            <div>
              <label className="block text-xs font-medium text-ink mb-1">Desde</label>
              <input
                type="month"
                required
            ...
            <div>
              <label className="block text-xs font-medium text-ink mb-1">Hasta</label>
              <input
                type="month"
                disabled={values.isCurrent}
                value={values.endMonth}
                onChange={(e) => setValues((v) => ({ ...v, endMonth: e.target.value }))}
                className="w-full px-3 py-2 rounded-[8px] border border-border text-sm text-ink bg-white focus:outline-none focus:border-brand disabled:opacity-50"
              />
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              type="submit"
              disabled={submitting}
              className="py-2 px-4 rounded-[8px] bg-brand hover:bg-brand-hover text-white text-sm font-semibold transition-colors disabled:opacity-60"
            >
            ...
            <button
              type="button"
              onClick={cancel}
              className="py-2 px-4 rounded-[8px] border border-border text-sm text-ink-secondary hover:border-brand"
            >
              Cancelar
            </button>
          </div>
