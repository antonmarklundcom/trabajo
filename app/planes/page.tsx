  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="text-center mb-12">
        <h1 className="text-3xl sm:text-4xl font-bold text-[#16181D]">Planes para empleadores</h1>
        <p className="mt-4 text-base text-[#5B6472] max-w-xl mx-auto">
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`bg-white rounded-[10px] border p-6 flex flex-col ${
                plan.featured
                  ? 'border-[#2557D6] shadow-sm ring-2 ring-[#2557D6]/20'
                  : 'border-[#E5E7EB]'
              }`}
            >
              {plan.featured && (
                <div className="mb-4">
                  <span className="text-xs font-semibold px-3 py-1 rounded-full bg-[#EEF3FE] text-[#2557D6]">
                    Más popular
                  </span>
                </div>
              )}
              <h2 className="text-xl font-bold text-[#16181D]">{plan.name}</h2>
              <div className="mt-3 mb-4">
                <span className="text-3xl font-bold text-[#16181D]">{plan.price}</span>
                <span className="text-sm text-[#5B6472] ml-1">{plan.priceNote}</span>
              </div>
              <p className="text-sm text-[#5B6472] mb-6">{plan.description}</p>
                ...
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm text-[#374151]">
                    <svg
                      ...
                      viewBox="0 0 20 20"
                      fill="currentColor"
                      className="text-[#16794C] flex-shrink-0 mt-0.5"
                    >
                      <path
                        fillRule="evenodd"
              ...
              <Link
                ...
                className={`mt-8 w-full py-3 px-4 rounded-[10px] text-center font-semibold text-sm transition-colors ${
                  plan.featured
                    ? 'bg-[#2557D6] hover:bg-[#1E47B8] text-white'
                    : 'border-2 border-[#2557D6] text-[#2557D6] hover:bg-[#EEF3FE]'
                }`}
              >
                {plan.cta}
              </Link>
      ...
      {/* FAQ */}
      <div className="mt-16">
        <h2 className="text-2xl font-bold text-[#16181D] mb-6">Preguntas frecuentes</h2>
        <div className="space-y-4">
          {[
            ...
            <div key={item.q} className="bg-white rounded-[10px] border border-[#E5E7EB] p-5">
              <h3 className="font-semibold text-[#16181D] mb-2">{item.q}</h3>
