...
export default async function MisDatosPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-ink">Mis datos</h1>
      ...
      <section className="bg-white rounded-[10px] border border-border p-6">
        ...
        <a
          ...
          className="mt-4 inline-block px-4 py-2 rounded-[10px] text-sm font-medium text-white bg-brand hover:bg-[#A32C22] transition-colors"
        >
          Descargar mis datos (JSON)
        </a>
        <p className="text-xs text-ink-3 mt-3">
          Los archivos de CV no van dentro del JSON: los descargás desde{' '}
          <Link href="/postulante/perfil" className="underline hover:text-brand">
            Mi perfil
          </Link>
          .
        </p>
      ...
      {/* --- Rectificación ------------------------------------------------ */}
      <section className="bg-white rounded-[10px] border border-border p-6">
        <h2 className="text-lg font-semibold text-ink">Corregir mis datos</h2>
        <p className="text-sm text-ink-secondary mt-1">
        ...
        <Link
          ...
          className="mt-4 inline-block px-4 py-2 rounded-[10px] text-sm font-medium text-ink border border-border hover:border-brand hover:text-brand transition-colors"
        >
          Ir a Mi perfil
        </Link>
        <p className="text-xs text-ink-3 mt-3">
          Para cambiar el email de tu cuenta, escribinos desde{' '}
          <Link href="/contacto" className="underline hover:text-brand">
            Contacto
          </Link>
          .
        </p>
            ...
            {active.map((app) => (
              <li key={app.id} className="py-3 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-medium text-ink truncate">{app.jobTitle}</p>
      ...
      <section className="bg-white rounded-[10px] border border-[#F3C9C4] p-6">
        <h2 className="text-lg font-semibold text-error">Eliminar mi cuenta</h2>
        <p className="text-sm text-ink-secondary mt-1">
          Eliminamos de verdad: no marcamos tu cuenta como &ldquo;inactiva&rdquo;, la borramos.
        </p>

        <ul className="mt-4 space-y-1.5 text-sm text-ink-secondary list-disc pl-5">
