...
function ReasonGate({ candidateId, invalid }: { candidateId: number; invalid: boolean }) {
  return (
    <div className="max-w-xl space-y-6">
      <div>
        <Link href="/admin/postulantes" className="text-sm text-ink-secondary hover:text-brand">
          ← Postulantes
        </Link>
        <h1 className="mt-2 text-2xl font-bold text-ink">Postulante #{candidateId}</h1>
      ...
      <form method="GET" className="bg-white rounded-[10px] border border-border p-6 space-y-4">
        {invalid && (
        ...
        <fieldset className="space-y-2">
          <legend className="text-sm font-medium text-ink mb-2">Motivo del acceso</legend>
          {ACCESS_REASON_CODES.map((code, index) => (
            <label key={code} className="flex items-center gap-2 text-sm text-ink-secondary">
              <input
                type="radio"
                name="motivo"
                value={code}
        ...
        <div>
          <label htmlFor="detalle" className="block text-sm font-medium text-ink mb-1">
            Detalle <span className="text-ink-3 font-normal">(obligatorio si elegís Otro)</span>
          </label>
          <input
            id="detalle"
            name="detalle"
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-2xl font-bold text-ink">Postulante #{profile.id}</h1>
      </div>

      <div className="rounded-[10px] border border-[#F3C9C4] bg-brand-tint px-4 py-3">
      ...
      <section className="bg-white rounded-[10px] border border-border p-6">
        <h2 className="text-sm font-semibold text-ink mb-4">Datos personales</h2>
        <dl className="grid sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
          <div>
            <dt className="text-xs text-ink-3">Nombre</dt>
            <dd className="text-ink font-medium">{profile.name}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-3">Email</dt>
            <dd className="text-ink-secondary">{profile.email}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-3">Teléfono</dt>
            <dd className="text-ink-secondary">{profile.phone}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-3">Ciudad</dt>
            <dd className="text-ink-secondary">{profile.cityName ?? 'Sin ciudad'}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-3">Titular</dt>
            <dd className="text-ink-secondary">{profile.headline ?? '—'}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-3">Estado</dt>
            <dd className="text-ink-secondary">{profile.isActive ? 'Activa' : 'Inactiva'}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-xs text-ink-3">Último ingreso</dt>
            <dd className="text-ink-secondary">
              {profile.lastLoginAt
      ...
      <section className="bg-white rounded-[10px] border border-border p-6">
        <h2 className="text-sm font-semibold text-ink mb-1">CVs</h2>
        <p className="text-xs text-ink-3 mb-4">
          Abrir un CV es un acceso aparte y se registra por separado.
        </p>
        {profile.cvs.length === 0 ? (
          <p className="text-sm text-ink-secondary">No subió ningún CV.</p>
        ) : (
          <ul className="divide-y divide-border border-t border-border">
            {profile.cvs.map((cv) => (
              <li key={cv.id} className="py-3 flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm text-ink truncate">{cv.originalFilename}</p>
      ...
      <section className="bg-white rounded-[10px] border border-border p-6">
        <h2 className="text-sm font-semibold text-ink mb-4">Experiencia laboral</h2>
        {profile.experiences.length === 0 ? (
          <p className="text-sm text-ink-secondary">Sin experiencia cargada.</p>
        ) : (
          <ul className="space-y-3">
            {profile.experiences.map((exp) => (
              <li key={exp.id} className="text-sm">
                <p className="font-medium text-ink">{exp.title}</p>
                <p className="text-ink-secondary">{exp.companyName}</p>
                <p className="text-xs text-ink-3">
      ...
      <section className="bg-white rounded-[10px] border border-border p-6">
        ...
        {profile.applications.length === 0 ? (
          <p className="text-sm text-ink-secondary">Todavía no se postuló a ningún empleo.</p>
        ) : (
          <ul className="divide-y divide-border border-t border-border">
            {profile.applications.map((app) => (
              <li key={app.id} className="py-3 flex items-center justify-between gap-4 text-sm">
                <div className="min-w-0">
                  <p className="text-ink truncate">{app.jobTitle}</p>
                  <p className="text-ink-secondary">{app.companyName}</p>
                  {app.redactedAt && (
                ...
                <div className="flex-shrink-0 text-right">
                  <p className="text-ink-secondary">{STATUS_LABEL[app.status] ?? app.status}</p>
                  <p className="text-xs text-ink-3">
